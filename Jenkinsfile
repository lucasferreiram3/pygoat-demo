    ...
    stages {
        stage('Archive') { 
            steps {
                ...
                sh 'find "${WORKSPACE}" -name "*.py*" -o -name "*.html*" -o -name "*.js*" -o -name "*.css*" -o -name "*.json*" -o -name "*.lock*" | tar --exclude=./uploadToVeracode --exclude=./*.git* --exclude=./*.github*  --exclude=./*test* --exclude=./*mock* --exclude=./*md* --exclude=./*svg* --exclude=./*png* -cvzf uploadToVeracode/upload.tar.gz .'
            }
        }
